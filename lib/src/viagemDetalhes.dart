...
class _ViagemDetalhesScreenState extends State<ViagemDetalhesScreen> {
  ...
  Map<String, dynamic> _getStatusInfo(String? status) {
    final statusViagem = widget.viagemData['status_rota']?.toString();
    Color statusColor;
    String statusTexto;

    switch (statusViagem?.toLowerCase()) {
      case 'aprovado':
      case 'finalizada':
        statusColor = Colors.green.shade400;
        statusTexto = statusViagem!;
        break;
      case 'reprovado':
  ...
  Widget build(BuildContext context) {
    final statusInfo = _getStatusInfo(
      widget.viagemData['status_rota']?.toString(),
    );
    final statusTexto = statusInfo['text'];
    final statusColor = statusInfo['color'];
    final isReprovado = statusTexto.toLowerCase() == 'reprovado';
    final motivoReprovacao = widget.viagemData['motivo_reprovacao']?.toString();

    double totalDistanceKm = 0;
    int totalSteps = 0;
    if (!_isLoading && _steps.isNotEmpty) {
      totalDistanceKm = _steps.fold(0.0, (prev, step) => prev + step.distance);
  ...
  Widget _buildBody(
    ...
    if (_rotaData == null || _steps.isEmpty) {
      return const Center(
        child: Padding(
          padding: EdgeInsets.all(20.0),
          child: Text(
            'Não foram encontrados dados detalhados para esta rota.',
    ...
    return ListView(
      ...
      children: [
        _buildStatusChip(statusTexto, statusColor),
        if (isReprovado &&
            motivoReprovacao != null &&
            motivoReprovacao.isNotEmpty)
          _buildInfoCard(
            context: context,
        ...
        _buildInfoCard(
          ...
          title: 'Condutor',
          value:
              (widget.viagemData['motorista']?['codigo_motorista'] != null &&
                      widget.viagemData['motorista']?['nome_motorista'] != null)
                  ? '${widget.viagemData['motorista']['codigo_motorista']} - ${widget.viagemData['motorista']['nome_motorista']}'
                  : 'Não informado',
        ),
        const SizedBox(height: 20),
